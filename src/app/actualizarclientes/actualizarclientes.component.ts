...
@Component({
  ...
  styleUrls: ['./actualizarclientes.component.css']
})
export class ActualizarclientesComponent implements OnInit {
  ...
  cliente: any = {};
  ngOnInit(): void {
    let temporal: any = localStorage.getItem('cliente');
...
resultadoServicio(res:any){
  this.cargar=false;
  console.log(res);
  alert("Cliente Guardada con el Id"+res.id_cliente)
}
servicioGuardar() {
  var httpOptions = {
  ...
  return this.http
    .post<any>(
      'http://localhost:8585/cliente/guardar',
